...
import { getEligibleVaccination } from "../graphql/queries";
import { updateEligibleVaccination } from "../graphql/mutations";
export default function EligibleVaccinationUpdateForm(props) {
  const {
    id: idProp,
    eligibleVaccination: eligibleVaccinationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = eligibleVaccinationRecord
      ? { ...initialValues, ...eligibleVaccinationRecord }
      : initialValues;
    setOsID(cleanValues.osID);
    setClientEligibleID(cleanValues.clientEligibleID);
    setClientID(cleanValues.clientID);
    setProfissionalID(cleanValues.profissionalID);
    setProfissionalDoc(cleanValues.profissionalDoc);
    ...
    setLocalCity(cleanValues.localCity);
    setLocalState(cleanValues.localState);
    setErrors({});
  };
  const [eligibleVaccinationRecord, setEligibleVaccinationRecord] =
    React.useState(eligibleVaccinationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getEligibleVaccination.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEligibleVaccination
        : eligibleVaccinationModelProp;
      setEligibleVaccinationRecord(record);
    };
    queryData();
  }, [idProp, eligibleVaccinationModelProp]);
  React.useEffect(resetStateValues, [eligibleVaccinationRecord]);
  const validations = {
    osID: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          profissionalDoc: profissionalDoc ?? null,
          coren: coren ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateEligibleVaccination.replaceAll("__typename", ""),
            variables: {
              input: {
                id: eligibleVaccinationRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EligibleVaccinationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Os id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || eligibleVaccinationModelProp)}
