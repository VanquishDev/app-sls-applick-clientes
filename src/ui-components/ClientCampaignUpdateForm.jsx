...
import { getClientCampaign } from "../graphql/queries";
import { updateClientCampaign } from "../graphql/mutations";
export default function ClientCampaignUpdateForm(props) {
  const {
    id: idProp,
    clientCampaign: clientCampaignModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = clientCampaignRecord
      ? { ...initialValues, ...clientCampaignRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setClientNotes(cleanValues.clientNotes);
    setInternalNotes(cleanValues.internalNotes);
    ...
    setScheduleFinished(cleanValues.scheduleFinished);
    setTotalEligibles(cleanValues.totalEligibles);
    setTotalEligiblesDependent(cleanValues.totalEligiblesDependent);
    setTotalEligiblesThird(cleanValues.totalEligiblesThird);
    setTotalVaccinations(cleanValues.totalVaccinations);
    setTotalVaccinationsDependent(cleanValues.totalVaccinationsDependent);
    setTotalVaccinationsThird(cleanValues.totalVaccinationsThird);
    setSearch(cleanValues.search);
    setResponsible(cleanValues.responsible);
    ...
    setAllowOffList(cleanValues.allowOffList);
    setLeafDiscount(cleanValues.leafDiscount);
    setIdx(cleanValues.idx);
    setCampaignCode(cleanValues.campaignCode);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [clientCampaignRecord, setClientCampaignRecord] = React.useState(
    clientCampaignModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getClientCampaign.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getClientCampaign
        : clientCampaignModelProp;
      setClientCampaignRecord(record);
    };
    queryData();
  }, [idProp, clientCampaignModelProp]);
  React.useEffect(resetStateValues, [clientCampaignRecord]);
  const validations = {
    name: [{ type: "Required" }],
    description: [],
    clientNotes: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          scheduleFinished: scheduleFinished ?? null,
          totalEligibles: totalEligibles ?? null,
          totalEligiblesDependent: totalEligiblesDependent ?? null,
          totalEligiblesThird: totalEligiblesThird ?? null,
          totalVaccinations: totalVaccinations ?? null,
          totalVaccinationsDependent: totalVaccinationsDependent ?? null,
          totalVaccinationsThird: totalVaccinationsThird ?? null,
          search: search ?? null,
          ...
          allowOffList: allowOffList ?? null,
          leafDiscount: leafDiscount ?? null,
          idx,
          campaignCode,
          status: status ?? null,
        };
        ...
        try {
          ...
          await API.graphql({
            query: updateClientCampaign.replaceAll("__typename", ""),
            variables: {
              input: {
                id: clientCampaignRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ClientCampaignUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || clientCampaignModelProp)}
