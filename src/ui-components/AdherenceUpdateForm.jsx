...
import { getAdherence } from "../graphql/queries";
import { updateAdherence } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function AdherenceUpdateForm(props) {
  const {
    id: idProp,
    adherence: adherenceModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = adherenceRecord
      ? { ...initialValues, ...adherenceRecord }
      : initialValues;
  ...
  const [adherenceRecord, setAdherenceRecord] =
    React.useState(adherenceModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getAdherence.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAdherence
        : adherenceModelProp;
    ...
    queryData();
  }, [idProp, adherenceModelProp]);
  React.useEffect(resetStateValues, [adherenceRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateAdherence.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "AdherenceUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || adherenceModelProp)}
