...
import { getAuthorizationListMember } from "../graphql/queries";
import { updateAuthorizationListMember } from "../graphql/mutations";
export default function AuthorizationListMemberUpdateForm(props) {
  const {
    id: idProp,
    authorizationListMember: authorizationListMemberModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = authorizationListMemberRecord
      ? { ...initialValues, ...authorizationListMemberRecord }
      : initialValues;
    ...
    setOthers(cleanValues.others);
    setErrors({});
  };
  const [authorizationListMemberRecord, setAuthorizationListMemberRecord] =
    React.useState(authorizationListMemberModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getAuthorizationListMember.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAuthorizationListMember
        : authorizationListMemberModelProp;
      setAuthorizationListMemberRecord(record);
    };
    queryData();
  }, [idProp, authorizationListMemberModelProp]);
  React.useEffect(resetStateValues, [authorizationListMemberRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateAuthorizationListMember.replaceAll("__typename", ""),
            variables: {
              input: {
                id: authorizationListMemberRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AuthorizationListMemberUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || authorizationListMemberModelProp)}
