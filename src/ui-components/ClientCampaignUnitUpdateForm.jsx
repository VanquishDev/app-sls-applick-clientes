...
import { getClientCampaignUnit } from "../graphql/queries";
import { updateClientCampaignUnit } from "../graphql/mutations";
export default function ClientCampaignUnitUpdateForm(props) {
  const {
    id: idProp,
    clientCampaignUnit: clientCampaignUnitModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = clientCampaignUnitRecord
    ...
    setTotalContractedVaccines(cleanValues.totalContractedVaccines);
    setQtyVisits(cleanValues.qtyVisits);
    setQtyVisitsConfirmed(cleanValues.qtyVisitsConfirmed);
    setQtyProfessional(cleanValues.qtyProfessional);
    setServicePoints(cleanValues.servicePoints);
    setServiceResponsible(cleanValues.serviceResponsible);
    setVaccination(cleanValues.vaccination);
    setTypeService(cleanValues.typeService);
    setIdx(cleanValues.idx);
    setRoutingRegionID(cleanValues.routingRegionID);
    setCode(cleanValues.code);
    setErrors({});
  };
  const [clientCampaignUnitRecord, setClientCampaignUnitRecord] =
    React.useState(clientCampaignUnitModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getClientCampaignUnit.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getClientCampaignUnit
        : clientCampaignUnitModelProp;
      setClientCampaignUnitRecord(record);
    };
    queryData();
  }, [idProp, clientCampaignUnitModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          totalContractedVaccines: totalContractedVaccines ?? null,
          qtyVisits: qtyVisits ?? null,
          qtyVisitsConfirmed: qtyVisitsConfirmed ?? null,
          qtyProfessional: qtyProfessional ?? null,
          servicePoints: servicePoints ?? null,
          serviceResponsible: serviceResponsible ?? null,
          vaccination: vaccination ?? null,
          typeService: typeService ?? null,
          idx,
          routingRegionID: routingRegionID ?? null,
          code: code ?? null,
        };
        ...
        try {
          ...
          await API.graphql({
            query: updateClientCampaignUnit.replaceAll("__typename", ""),
            variables: {
              input: {
                id: clientCampaignUnitRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ClientCampaignUnitUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || clientCampaignUnitModelProp)}
