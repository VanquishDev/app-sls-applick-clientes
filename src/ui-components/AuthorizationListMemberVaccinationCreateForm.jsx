...
import { createAuthorizationListMemberVaccination } from "../graphql/mutations";
export default function AuthorizationListMemberVaccinationCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: createAuthorizationListMemberVaccination.replaceAll(
              "__typename",
              ""
            ),
            variables: {
      ...
      {...getOverrideProps(
        overrides,
        "AuthorizationListMemberVaccinationCreateForm"
      )}
      {...rest}
    >
      <TextField
